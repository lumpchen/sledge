 ...
 * password-based decryption mechanisms, as described in section 3.5 of the PDF
...
public class StandardDecrypter implements PDFDecrypter {
	...
	/**
	 * Padding used to bring passwords up to 32 bytes, as specified by the first
	 * step of Algorithm 3.2 in the PDF Reference version 1.7.
	 */
	private final static byte[] PW_PADDING = new byte[] { 0x28, (byte) 0xBF,
			0x4E, 0x5E, 0x4E, 0x75, (byte) 0x8A, 0x41, 0x64, 0x00, 0x4E, 0x56,
			(byte) 0xFF, (byte) 0xFA, 0x01, 0x08, 0x2E, 0x2E, 0x00,
			(byte) 0xB6, (byte) 0xD0, 0x68, 0x3E, (byte) 0x80, 0x2F, 0x0C,
			(byte) 0xA9, (byte) 0xFE, 0x64, 0x53, 0x69, 0x7A };
	 ...
	 *            dictionary; can be null, but according to the spec, shouldn't
	 *            be. Is expected to be an array of two byte sequences.
	 * @param keyBitLength
	 *            the length of the key in bits; should be a multiple of 8
	 *            between 40 and 128
	 * @param revision
	 *            the revision of the Standard encryption security handler being
	 *            employed. Should be 2, 3 or 4.
	 * @param oValue
	 ...
	 *            whether metadata is being encrypted, as identified by the
	 *            Encrypt dict (with default true if not explicitly identified)
	 * @param password
	 *            the password; not null
	 * @throws IOException
	 *             if there's a problem reading the file
	 * @throws EncryptionUnsupportedByPlatformException
	 *             if the encryption is not supported by the environment in
	 *             which the code is executing
	 * @throws EncryptionUnsupportedByProductException
	 *             if PDFRenderer does not currently support the specified
	 *             encryption
	 * @throws PDFDecryptException
	 */
	public StandardDecrypter(EncryptionAlgorithm encryptionAlgorithm,
			PArray documentId, int keyBitLength, int revision, byte[] oValue,
			byte[] uValue, int pValue, boolean encryptMetadata,
			PDFPassword password) throws IOException,
			EncryptionUnsupportedByProductException,
			EncryptionUnsupportedByPlatformException, PDFDecryptException {
		...
		if (documentId == null) {
			...
			firstDocIdValue = PDFStringUtil.asBytes(documentId.get(0)
					.toString());
		}
		...
		try {
			final List<byte[]> passwordBytePossibilities = password
					.getPasswordBytes(false);
			for (int i = 0; generalKeyBytes == null
					&& i < passwordBytePossibilities.size(); ++i) {
				final byte[] passwordBytes = passwordBytePossibilities.get(i);
				generalKeyBytes = checkOwnerPassword(passwordBytes,
						firstDocIdValue, keyBitLength, revision, oValue,
						uValue, pValue, encryptMetadata);
				if (generalKeyBytes != null) {
					// looks like the password was the owner password!
					ownerAuthorised = true;
				} else {
					// try it as the user password
					generalKeyBytes = checkUserPassword(passwordBytes,
							firstDocIdValue, keyBitLength, revision, oValue,
							uValue, pValue, encryptMetadata);
			...
			throw new PDFDecryptException("Unable to check passwords: "
					+ e.getMessage(), e);
		}
	...
	public ByteBuffer decryptBuffer(String cryptFilterName,
		...
		if (cryptFilterName != null) {
			throw new PDFDecryptException(
					"This encryption version does not support Crypt filters");
		}

		if (streamObj != null) {
			checkNums(streamObj.getObjNum(), streamObj.getGenNumb());
		}

		final byte[] decryptionKeyBytes;
		if (streamObj == null) {
			// lack of a stream object indicates the unsalted key should be used
			decryptionKeyBytes = getUnsaltedDecryptionKey();
		} else {
			decryptionKeyBytes = getObjectSaltedDecryptionKey(
					streamObj.getObjNum(), streamObj.getGenNumb());
		}
		return decryptBuffer(stream, decryptionKeyBytes);
	}
	...
	public String decryptString(int objNum, int objGen, String inputBasicString)
		...
		final byte[] decryptionKey = getObjectSaltedDecryptionKey(objNum,
				objGen);
		final ByteBuffer decrypted = decryptBuffer(ByteBuffer.wrap(crypted),
				decryptionKey);
		return PDFStringUtil.asBasicString(decrypted.array(),
				decrypted.arrayOffset(), decrypted.limit());
	}
	 ...
	 * @throws EncryptionUnsupportedByPlatformException
	 *             if the platform does not support the required ciphers and key
	 *             lengths
	 * @throws PDFParseException
	 *             if there's an internal error while testing cipher
	 *             availability
	 */
	private void testJceAvailability(int keyBitLength)
		...
		try {
			createAndInitialiseContentCipher(ByteBuffer.wrap(junkBuffer),
					junkKey);
		} catch (PDFDecryptException e) {
			throw new PDFDecryptException("Internal error; "
	...
	private ByteBuffer decryptBuffer(PStream stream, byte[] decryptionKeyBytes)
			throws PDFDecryptException {
		byte[] bytes = stream.getStream();
		ByteBuffer encrypted = ByteBuffer.wrap(bytes);
		return this.decryptBuffer(encrypted, decryptionKeyBytes);
	}
	 ...
	 */
	private ByteBuffer decryptBuffer(ByteBuffer encrypted,
			byte[] decryptionKeyBytes) throws PDFDecryptException {

		final Cipher cipher;
		try {
			cipher = createAndInitialiseContentCipher(encrypted,
					decryptionKeyBytes);
		} catch (GeneralSecurityException e) {
		...
		try {
			...
			final ByteBuffer decryptedBuf = ByteBuffer.allocate(encrypted
					.remaining());
			cipher.doFinal(encrypted, decryptedBuf);
			decryptedBuf.flip();
			return decryptedBuf;
		} catch (GeneralSecurityException e) {
			throw new PDFDecryptException("Could not decrypt: "
					+ e.getMessage(), e);
		}
	}
	 ...
	 * 
	 * @param encrypted
	 *            the encrypted content; required by AES encryption so that the
	 *            initialisation vector can be established
	 * @param decryptionKeyBytes
	 *            the bytes for the decryption key
	 * @return a content decryption cypher, ready to accept input
	 * @throws PDFParseException
	 *             if the encrypted buffer is malformed or on an internal error
	 * @throws NoSuchAlgorithmException
	 *             if the cipher algorithm is not supported by the platform
	 * @throws NoSuchPaddingException
	 *             if the cipher padding is not supported by the platform
	 * @throws InvalidKeyException
	 *             if the key is invalid according to the cipher, or too long
	 * @throws InvalidAlgorithmParameterException
	 *             if the cipher parameters are bad
	 */
	private Cipher createAndInitialiseContentCipher(ByteBuffer encrypted,
			byte[] decryptionKeyBytes) throws PDFDecryptException,
		...
		if (encryptionAlgorithm.isRC4()) {
			...
			final SecretKeySpec aesKey = new SecretKeySpec(decryptionKeyBytes,
					KEY_AES);
			final IvParameterSpec aesIv = new IvParameterSpec(
	...
	private byte[] getObjectSaltedDecryptionKey(int objNum, int objGen)
		...
		try {
			...
			throw new PDFDecryptException("Unable to get MD5 digester", e);
		}
	 ...
	 *            the object generation
	 * @throws PDFParseException
	 *             if the object numbering indicates that they aren't true
	 *             object numbers
	 */
	private void checkNums(int objNum, int objGen) throws PDFDecryptException {
		if (objNum < 0) {
			throw new PDFDecryptException(
	 ...
	 * document configuration. Correponds to Algorithms 3.4 and 3.5 of the PDF
	 * Reference version 1.7
	 * 
	 * @param generalKey
	 *            the general encryption key
	 * @param firstDocIdValue
	 *            the value of the first element in the document's ID entry in
	 *            the trailer dictionary
	 * @param revision
	 *            the revision of the security handler
	 * @return the U value for the given configuration
	 * @throws GeneralSecurityException
	 *             if there's an error getting required ciphers, etc.
	 *             (unexpected, since a check for algorithm availability is
	 *             performed on construction)
	 * @throws EncryptionUnsupportedByProductException
	 *             if the revision is not supported
	 */
	private byte[] calculateUValue(byte[] generalKey, byte[] firstDocIdValue,
			int revision) throws GeneralSecurityException,
			EncryptionUnsupportedByProductException {
	 ...
	 * process is reversed to determine whether a given password is the owner
	 * password. Corresponds to Algorithm 3.3 of the PDF Reference version 1.7.
	 * 
	 ...
	 * @throws GeneralSecurityException
	 *             if ciphers are unavailable or inappropriately used
	 */
	private byte[] calculuateOValue(byte[] ownerPassword, byte[] userPassword,
			int keyBitLength, int revision) throws GeneralSecurityException {

		// Steps 1-4
		final byte[] rc4KeyBytes = getInitialOwnerPasswordKeyBytes(
	 ...
	 * 
	 * @param ownerPassword
	 *            the suggested owner password (may be null or empty)
	 * @param firstDocIdValue
	 *            the byte stream from the first element of the value of the ID
	 ...
	 * @param encryptMetadata
	 *            the EncryptMetadata entry from the Encrypt dictionary (or
	 *            false if not present or revision &lt;= 3)
	 * @return the general/user key bytes if the owner password is currect,
	 *         <code>null</code> otherwise
	 * @throws GeneralSecurityException
	 *             if there's a problem with cipher or digest usage; unexpected
	 * @throws EncryptionUnsupportedByProductException
	 *             if PDFRenderer doesn't support the security handler revision
	 * @throws PDFParseException
	 *             if the document is malformed
	 */
	private byte[] checkOwnerPassword(byte[] ownerPassword,
			byte[] firstDocIdValue, int keyBitLength, int revision,
			byte[] oValue, byte[] uValue, int pValue, boolean encryptMetadata)
			throws GeneralSecurityException,
			EncryptionUnsupportedByProductException, PDFDecryptException {
		...
		if (revision == 2) {
			...
			// value of the encryption dictionaryâs O entry (first iteration)
			// or
		...
		return checkUserPassword(possibleUserPassword, firstDocIdValue,
				keyBitLength, revision, oValue, uValue, pValue, encryptMetadata);

	}

	/**
	 * Establish the key to be used for the generation and validation of the
	 * user password via the O entry. Corresponds to steps 1-4 in Algorithm 3.3
	 * of the PDF Reference version 1.7.
	 * 
	 * @param ownerPassword
	 *            the owner password
	 * @param keyBitLength
	...
	/**
	 * Check to see whether a provided user password is correct with respect to
	 * an Encrypt dict configuration. Corresponds to algorithm 3.6 of the PDF
	 * Reference version 1.7
	 * 
	 * @param userPassword
	...
	private byte[] checkUserPassword(byte[] userPassword,
		...
		final byte[] generalKey = calculateGeneralEncryptionKey(userPassword,
				firstDocIdValue, keyBitLength, revision, oValue, pValue,
				encryptMetadata);
		// Algorithm 3.4/5,RemainingSteps:
		final byte[] calculatedUValue = calculateUValue(generalKey,
				firstDocIdValue, revision);
		...
		if (uValue.length != calculatedUValue.length) {
			throw new PDFDecryptException("Improper U entry length; "
	 ...
	 * @throws GeneralSecurityException
	 *             if an error occurs when obtaining and operating
	 *             ciphers/digests
	 */
	private byte[] calculateGeneralEncryptionKey(byte[] userPassword,
			byte[] firstDocIdValue, int keyBitLength, int revision,
			byte[] oValue, int pValue, boolean encryptMetadata)
			throws GeneralSecurityException {
		...
		// Step 3: Pass the value of the encryption dictionaryâs O entry to
		// the
		...
		// (the value of the ID entry in the documentâs trailer dictionary;
		// see
	...
	private byte[] padPassword(byte[] password) {
		...
		// â n bytes of the padding string to the end of the password string.
		// If
		...
		final int numContributingPasswordBytes = password.length > padded.length ? padded.length
				: password.length;
	...
	/**
	 * Reverse the {@link #rc4shuffle} operation, and the operation that
	 * invariable preceeds it, thereby obtaining an original message
	 * 
	 * @param rc4
	 ...
	 * 
	 * @param rc4
	 *            the cipher to use; must be a stream cipher producing identical
	 *            output length to input (e.g., RC4)
	 * @param buffer
	 ...
	 *            the cipher key
	 * @throws InvalidKeyException
	 *             if the key is of an unacceptable size or doesn't belong to
	 *             the cipher
	 */
	 ...
	 */
	private Cipher createRC4Cipher() throws NoSuchAlgorithmException,
	 ...
	 */
	private Cipher createAESCipher() throws NoSuchAlgorithmException,
			NoSuchPaddingException {
	 ...
	 * @throws GeneralSecurityException
	 *             if there's a problem hashing; e.g., if the buffer is too
	 *             small
	 */
