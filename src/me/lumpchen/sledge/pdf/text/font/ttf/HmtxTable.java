...
public class HmtxTable extends TrueTypeTable {
	...
	public void setData(ByteBuffer data) {
		// some PDF writers subset the font but don't update the number of
		// glyphs in the maxp table,
		// this would appear to break the TTF spec.
		// A better solution might be to try and override the numGlyphs in the
		// maxp table based
		// on the number of entries in the cmap table or by parsing the glyf
		// table, but this
		// appears to be the only place that gets affected by the discrepancy...
		// so far!...
		...
		if (i < leftSideBearings.length) {
			Arrays.fill(leftSideBearings, i, leftSideBearings.length - 1,
					(short) 0);
		}
	}
