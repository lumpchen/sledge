 ...
 * (if that) are represented in the PDFDocEncoding, so users should be prepared
 * to deal with unmappable character exceptions.
 * 
...
public class PDFDocCharsetEncoder extends CharsetEncoder {
	...
	/**
	 * For each character that exists in PDFDocEncoding, identifies whether the
	 * byte value in UTF-16BE is the same as it is in PDFDocEncoding
	 */
	final static boolean[] IDENT_PDF_DOC_ENCODING_MAP = new boolean[256];

	/**
	 * For non-identity encoded characters, maps from the character to the byte
	 * value in PDFDocEncoding. If an entry for a non-identity coded character
	 * is absent from this map, that character is unmappable in the
	 * PDFDocEncoding.
	 */
	final static Map<Character, Byte> EXTENDED_TO_PDF_DOC_ENCODING_MAP = new HashMap<Character, Byte>();
	static {
		for (int i = 0; i < PDFStringUtil.PDF_DOC_ENCODING_MAP.length; ++i) {
			...
			if (!identical) {
				EXTENDED_TO_PDF_DOC_ENCODING_MAP.put(c, (byte) i);
			}
		}
	}
