 ...
 * 
 * <p>
 * In revisions up to version 1.e Expansion 3, the mapping between a string and
 * the bytes corresponding to the password was poorly specified, meaning that
 * the safest manner in which to specify a password was via a byte array. With
 * 1.7 expansion 3, a still slightly problematic mapping was given for the
...
public class PDFPassword {
	...
	/** The empty password */
	public static final PDFPassword EMPTY_PASSWORD = new PDFPassword(
			new byte[0]);
	...
	public PDFPassword(byte[] passwordBytes) {
		this.passwordBytes = passwordBytes != null ? passwordBytes
				: new byte[0];
	}
	...
	List<byte[]> getPasswordBytes(boolean unicodeConversion) {
		...
		if (this.passwordBytes != null || this.passwordString == null) {
			...
			if (isAlphaNum7BitString(this.passwordString)) {
				...
				// in any other way
				return Collections.singletonList(PDFStringUtil
						.asBytes(passwordString));
			} else {
	 ...
	 */
	private static abstract class CharsetEncoderGenerator implements
			PasswordByteGenerator {
	 ...
	 */
	private static class PDFDocEncodingByteGenerator extends
	 ...
	 */
	private static class IdentityEncodingByteGenerator extends
			CharsetEncoderGenerator {
