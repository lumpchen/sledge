...
public abstract class Predictor {
	 ...
	 */
	public static Predictor getPredictor(PDictionary decodeParms)
			throws IOException {
		// get the algorithm (required)
		PNumber algorithmObj = decodeParms.getValueAsNumber(PName.Predictor);
		if (algorithmObj == null) {
			return null;
		}

		int algorithm = algorithmObj.intValue();
		...
		case 15:
			...
			break;
		default:
			throw new DecodeException("Unknown predictor: " + algorithm);
		}

		// read the colors (optional)
		PNumber colorsObj = decodeParms.getValueAsNumber(PName
				.instance("Colors"));
		if (colorsObj != null) {
			predictor.setColors(colorsObj.intValue());
		}

		// read the bits per component (optional)
		PNumber bpcObj = decodeParms.getValueAsNumber(PName
				.instance("BitsPerComponent"));
		if (bpcObj != null) {
			predictor.setBitsPerComponent(bpcObj.intValue());
		}

		// read the columns (optional)
		PNumber columnsObj = decodeParms.getValueAsNumber(PName
				.instance("Columns"));
		if (columnsObj != null) {
			predictor.setColumns(columnsObj.intValue());
		}
